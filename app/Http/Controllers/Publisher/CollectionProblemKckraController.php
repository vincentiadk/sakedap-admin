...
class CollectionProblemKckraController extends Controller
{
    ...
    public function index($connect = null)
    {
        ...
        $data = [
            ...
            'content' => 'publisher.collection.problem_kckra',
    ...
    public function updateCopyRejected(Request $request, $id)
    {
        try {
            $copyRejected = CopyRejected::find($id);
            $copyRejected->handling = $request->handling;
            $copyRejected->save();

            session()->flash('success', 'Berhasil disimpan!');
            $response = ['status' => 200, 'id' => $copyRejected->id];
        } catch (\Exception $e) {
            $data[] = $e->getMessage();
            $response = [
                'status' => 422,
