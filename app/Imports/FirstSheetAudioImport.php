...
use App\Models\Collection as Audio;
...
class FirstSheetAudioImport implements ToCollection, WithHeadingRow
{
    private $folderName;
    private $userId;
    private $sessionName;
    ...
    private function createCollection($item, $publisher)
    {
        ...
        if ($item['isrc'] != '') {
            $partitur = Audio::where('type', 5)
                ->where('code', $item['isrc'])
        ...
        $coverUpload = Storage::disk($this->location->location)->path('public/collection/audio/cover/' . $collection->id . '/'  . $coverUploadName);
        $originalUpload = Storage::disk($this->location->location)->path('public/collection/audio/original/' . $collection->id . '/'  . $originalUploadName);

        if (!file_exists(Storage::disk($this->location->location)->path('public/collection/audio/cover/' . $collection->id))) {
            mkdir(Storage::disk($this->location->location)->path('public/collection/audio/cover/' . $collection->id), 0777, true);
        }

        if (!file_exists(Storage::disk($this->location->location)->path('public/collection/audio/original/' . $collection->id))) {
            mkdir(Storage::disk($this->location->location)->path('public/collection/audio/original/' . $collection->id), 0777, true);
        }
        ...
        CollectionMedia::insert([
            [
                'collection_id' => $collection->id,
                'link'          => 'public/collection/audio/cover/' . $coverUploadName,
            ...
            [
                'collection_id' => $collection->id,
                'link'          => 'public/collection/audio/original/' . $originalUploadName,
