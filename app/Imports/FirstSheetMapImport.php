...
use App\Models\Collection as Map;
...
class FirstSheetMapImport implements ToCollection, WithHeadingRow
{
    private $folderName;
    private $userId;
    private $sessionName;
    ...
    private function createCollection($item, $publisher)
    {
        ...
        if ($item['isbn'] != '') {
            $partitur = Map::where('type', 3)
                ->where('code', $item['isbn'])
        ...
        $coverUpload = Storage::disk($this->location->location)->path('public/collection/peta/cover/' . $collection->id . '/' . $coverUploadName);
        $originalUpload = Storage::disk($this->location->location)->path('public/collection/peta/original/' . $collection->id . '/' . $originalUploadName);

        if (!file_exists(Storage::disk($this->location->location)->path('public/collection/peta/cover/' . $collection->id))) {
            mkdir(Storage::disk($this->location->location)->path('public/collection/peta/cover/' . $collection->id), 0777, true);
        }

        if (!file_exists(Storage::disk($this->location->location)->path('public/collection/peta/original/' . $collection->id))) {
            mkdir(Storage::disk($this->location->location)->path('public/collection/peta/original/' . $collection->id), 0777, true);
        }
        ...
        CollectionMedia::insert([
            [
                'collection_id' => $collection->id,
                'link'          => 'public/collection/peta/cover/' . $collection->id . '/' . $coverUploadName,
            ...
            [
                'collection_id' => $collection->id,
                'link'          => 'public/collection/peta/original/' . $collection->id . '/' . $originalUploadName,
        ...
        GeneralHelper::pdfToImage('peta', 'public/collection/peta/original/' . $collection->id . '/' . $originalUploadName, $collection->id);
