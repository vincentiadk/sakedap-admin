...
use App\Models\Collection as Partitur;
...
class FirstSheetPartiturImport implements ToCollection, WithHeadingRow
{
    private $folderName;
    private $userId;
    private $sessionName;
    ...
    private function createCollection($item, $publisher)
    {
        ...
        if ($item['ismn'] != '') {
            $partitur = Partitur::where('type', 2)
                ->where('code', $item['ismn'])
        ...
        $coverUpload = Storage::disk($this->location->location)->path('public/collection/partitur/cover/' . $collection->id . '/'  . $coverUploadName);
        $originalUpload = Storage::disk($this->location->location)->path('public/collection/partitur/original/' . $collection->id . '/' . $originalUploadName);

        if (!file_exists(Storage::disk($this->location->location)->path('public/collection/partitur/cover/' . $collection->id))) {
            mkdir(Storage::disk($this->location->location)->path('public/collection/partitur/cover/' . $collection->id), 0777, true);
        }

        if (!file_exists(Storage::disk($this->location->location)->path('public/collection/partitur/original/' . $collection->id))) {
            mkdir(Storage::disk($this->location->location)->path('public/collection/partitur/original/' . $collection->id), 0777, true);
        }
        ...
        CollectionMedia::insert([
            [
                'collection_id' => $collection->id,
                'link'          => 'public/collection/partitur/cover/' . $collection->id . '/' . $coverUploadName,
            ...
            [
                'collection_id' => $collection->id,
                'link'          => 'public/collection/partitur/original/' . $collection->id . '/' . $originalUploadName,
        ...
        GeneralHelper::pdfToImage('partitur', 'public/collection/partitur/original/' . $collection->id . '/' . $originalUploadName, $collection->id);
