...
use App\Exports\CollectionKckraExport;
...
class DownloadReportCollectionKckra implements ShouldQueue
{
    ...
    public function __construct($data)
    {
        $this->prepareStatus();
        $this->data = $data;
        $this->setInput(['type' => 'Download Laporan Koleksi Kckra']);
    ...
    public function handle()
    {
        ...
        (new CollectionKckraExport($this->data))->store('public/excel/collection/kckra/' . date('d-m-Y') . '/' . $filename, $this->location->location);
        $this->setProgressNow(75);

        Download::create([
            'user_id'     => $this->data['user_id'],
            'slug'        => 'collection-kckra',
            'link'        => 'public/excel/collection/kckra/' . date('d-m-Y') . '/' . $filename,
            'description' => json_encode([
                ...
                'lib_loc_id'       => $this->data['lib_loc_id'],
