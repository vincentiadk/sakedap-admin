	...
	<div class="content-wrapper">
		<div class="content-header row">
			<div class="content-header-left col-md-6 col-12 mb-2">
				<h3 class="content-header-title mb-1 d-inline-block">Masalah Film</h3><br>
			...
			<section id="configuration">
				<div class="row">
					<div class="col-12">
									...
									<div class="row">
										<div class="col-md-12">
											<div class="form-group row">
												...
												<div class="col-md-10">
													<textarea name="title" id="title" class="form-control" style="resize:none;">{{ session('filter.collection.problem.6.title') }}</textarea>
												</div>
											</div>
										</div>
										<div class="col-md-12">
											<div class="form-group row">
												...
												<div class="col-md-10">
													...
													@if(!empty(session('filter.collection.problem.6.publisher_id')))
														<option value="{{session('filter.collection.problem.6.publisher_id')}}" selected="selected">{{  App\Models\Publisher::select('name')->where('id',session('filter.collection.problem.6.publisher_id'))->first()->name }}</option>
													@endif
													</select>
												</div>
											</div>
										...
										<div class="col-md-12">
											<div class="form-group row">
												...
												<div class="col-md-10">
													...
													@if(!empty(session('filter.collection.problem.6.province_id')))
														<option value="{{session('filter.collection.problem.6.province_id')}}" selected="selected">{{ App\Models\Province::where('id', session('filter.collection.problem.6.province_id'))->select('name')->first()->name }}</option>
													@endif
													</select>
												</div>
											</div>
										...
										<div class="col-md-12">
											<div class="form-group row">
												...
												<div class="col-md-10">
													...
													@if(!empty(session('filter.collection.problem.6.city')))
														<option value="{{session('filter.collection.problem.6.city')}}" selected="selected">{{ App\Models\City::select('name')->where('id',session('filter.collection.problem.6.city'))->first()->name }}</option>
													@endif
													</select>
												</div>
											</div>
										...
										<div class="col-md-12">
											<div class="form-group row">
												...
												<div class="col-md-10">
													<input type="number" name="publication_year" id="publication_year" class="form-control" value="{{ session('filter.collection.problem.6.publication_year') }}" >
												</div>
											</div>
										</div>
										<div class="col-md-12">
											<div class="form-group row">
												...
												<div class="col-md-10">
													<input type="text" name="code" id="code" class="form-control" value="{{ session('filter.collection.problem.6.code') }}">
												</div>
											</div>
										</div>
										<div class="col-md-12">
													...
													<div class="row">
														<div class="col-md-4">
															<select name="year_start" id="year_start" class="form-control">
																...
																@for($i = 2018; $i <= date('Y'); $i++)
																	<option value="{{ $i }}" {{ $i == session('filter.collection.problem.6.year_start') ? 'selected' : '' }}>{{ $i }}</option>
																@endfor
															</select>
														</div>
														<div class="col-md-1">
														...
														<div class="col-md-4">
															<select name="year_end" id="year_end" class="form-control">
																<option value="">-- Pilih --</option>
																@for($i = 2018; $i <= date('Y'); $i++)
																	<option value="{{ $i }}" {{ $i == session('filter.collection.problem.6.year_end') ? 'selected' : '' }}>{{ $i }}</option>
																@endfor
															</select>
														</div>
														<div class="col-md-3">
																...
																<div class="col-md-6">
																	...
																	<select name="month_start" id="month_start" class="form-control">
																		...
																		@foreach($month as $key =>$value)
																			<option value="{{$value}}" {{ $value == session('filter.collection.problem.6.month_start') ? 'selected' : '' }}>{{App\Helper\GeneralHelper::getMonth($value)}}</option>
																		@endforeach
																	</select>
																</div>
																<div class="col-md-6">
																	<select name="month_year_start" id="month_year_start" class="form-control">
																		<option value="">-- Pilih --</option>
																		@for($i = 2018; $i <= date('Y'); $i++)
																			<option value="{{ $i }}" {{ $i == session('filter.collection.problem.6.month_year_start') ? 'selected' : '' }}>{{ $i }}</option>
																		@endfor
																	</select>
																</div>
															</div>
															...
															<div class="row">
																<div class="col-md-6">
																	<select name="month_end" id="month_end" class="form-control">
																		<option value="">-- Pilih --</option>
																		@foreach($month as $key =>$value)
																			<option value="{{$value}}" {{ $value == session('filter.collection.problem.6.month_end') ? 'selected' : '' }}>{{App\Helper\GeneralHelper::getMonth($value)}}</option>
																		@endforeach
																	</select>
																</div>
																<div class="col-md-6">
																	<select name="month_year_end" id="month_year_end" class="form-control">
																		<option value="">-- Pilih --</option>
																		@for($i = 2018; $i <= date('Y'); $i++)
																			<option value="{{ $i }}" {{ $i == session('filter.collection.problem.6.month_year_end') ? 'selected' : '' }}>{{ $i }}</option>
																		@endfor
																	</select>
																</div>
															</div>
										...
										<div class="col-md-12">
											<div class="form-group row">
												...
												<div class="col-md-10">
													<div class="row">
														<div class="col-md-4">
															<input type="date" name="day_start" id="day_start" class="form-control" max="{{ date('Y-m-d') }}" value="{{ empty(session('filter.collection.problem.6.day_start'))? date('Y-m-d') : session('filter.collection.problem.6.day_start') }}">
														</div>
														<div class="col-md-1">
															<p style="line-height:40px;" class="text-center">s/d</p>
														</div>
														<div class="col-md-4">
															<input type="date" name="day_end" id="day_end" class="form-control" max="{{ date('Y-m-d') }}" value="{{ empty(session('filter.collection.problem.6.day_end'))? date('Y-m-d') : session('filter.collection.problem.6.day_end') }}">
														</div>
														<div class="col-md-3">
															<div class="form-group">
						...
						<div class="card">
							<div class="card-header">
								<h4 class="card-title">Daftar Masalah Film</h4>
							</div>
...
<script>
	$(function() {
		let param = "{{session('filter.collection.problem.6.param')}}"
	...
	function reset() {
		$.ajax({
          url: '{{ url("admin/collection/reset_filed/problem/6") }}',
          type: 'GET',
          contentType: false,
          processData: false,
	...
	function loadDataTable(param = '') {
		$('#datatable_serverside').DataTable({
			...
			pagingType: 'input',
			ajax: {
				url: '{{ url("admin/collection/problem/datatable/6") }}',
				data: {
					param: param,
					title: $('#title').val(),
