	...
	<div class="content-wrapper">
		<div class="content-header row">
			<div class="content-header-left col-md-6 col-12 mb-2">
				<h3 class="content-header-title mb-1 d-inline-block">Master Pimpinan / Director</h3><br>
				<div class="row breadcrumbs-top d-inline-block">
					<div class="breadcrumb-wrapper col-12">
						<ol class="breadcrumb">
							<li class="breadcrumb-item"><a href="{{ url('admin/dashboard') }}">Dashboard</a></li>
							<li class="breadcrumb-item"><a href="#">Master</a></li>
							<li class="breadcrumb-item active">Pimpinan / Director</li>
						</ol>
					</div>
				</div>
			</div>
					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<h4 class="card-title">Daftar Pimpinan / Director</h4>
							</div>
			...
			<div class="modal-body">
				...
				<form action="" id="form_data">
                    ...
                    <div class="form-group">
                        ...
                        <select class="select2" name="province_id" id="province_id" style="width:100%;">
                            <option value=""></option>
					...
					<div class="form-group">
						<label>Waktu Menjabat :</label>
						<input type="date" name="position_start" id="position_start" class="form-control">
					</div>
					<div class="form-group">
						<label>Jabatan Berakhir :</label>
						<input type="date" name="position_end" id="position_end" class="form-control">
					</div>
				</form>
			</div>
...
<script>
	...
	function toUpdate() {
        $('#position_end').attr('disabled', false);
	...
	function reset() {
        $('#position_end').attr('disabled', true);
	...
	function loadDataTable() {
		$('#datatable_serverside').DataTable({
			...
			pagingType : 'input',
			ajax: {
				url: '{{ url("admin/director/datatable") }}'
			},
			columns: [
				{
					name: 'id',
	...
	function show(id) {
		toUpdate();
		$.ajax({
			url: '{{ url("admin/director/show") }}' + '/' + id,
			type: 'GET',
			dataType: 'JSON',
			beforeSend: function() {
				loadingOpen('.modal-content');
			...
			success: function(response) {
				...
				$('#position_start').val(response.position_start);
				$('#position_end').val(response.position_end);
	...
	function update(id) {
		$.ajax({
			url: '{{ url("admin/director/update") }}' + '/' + id,
			type: 'POST',
			dataType: 'JSON',
			data: new FormData($('#form_data')[0]),
			cache: false,
	...
	function destroy(id) {
		Swal.fire({
			...
			if(result.value) {
				$.ajax({
					url: '{{ url("admin/director/destroy") }}' + '/' + id,
					type: 'GET',
					dataType: 'JSON',
					success: function(response) {
						if(response.status == 200) {
