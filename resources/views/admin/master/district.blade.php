 display nowrap<div class="app-content content">
  <div class="content-wrapper">
    <div class="content-header row">
      <div class="content-header-left col-md-6 col-12 mb-2">
        <h3 class="content-header-title mb-1 d-inline-block">Master Kecamatan</h3><br>
...
<script>
  ...
  function loadDataTable() {
    $('#datatable_serverside').DataTable({
      ...
      pagingType : 'input',
      ajax: {
        url: '{{ url("admin/district/datatable") }}'
      },
      columns: [
        {
          name: 'id',
  ...
  function show(id) {
    toUpdate();
    $.ajax({
      url: '{{ url("admin/district/show") }}' + '/' + id,
      type: 'GET',
      dataType: 'JSON',
      beforeSend: function() {
        loadingOpen('.modal-content');
  ...
  function update(id) {
    $.ajax({
      url: '{{ url("admin/district/update") }}' + '/' + id,
      type: 'POST',
      dataType: 'JSON',
      data: $('#form_data').serialize(),
      headers: {
  ...
  function destroy(id) {
    Swal.fire({
      ...
      if(result.value) {
        $.ajax({
          url: '{{ url("admin/district/destroy") }}' + '/' + id,
          type: 'GET',
          dataType: 'JSON',
          success: function(response) {
            if(response.status == 200) {
