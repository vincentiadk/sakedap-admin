    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-6 col-12 mb-2">
                <h3 class="content-header-title mb-1 d-inline-block">Master Library Location</h3><br>
                <div class="row breadcrumbs-top d-inline-block">
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ url('admin/dashboard') }}">Dashboard</a></li>
                            <li class="breadcrumb-item"><a href="#">Master</a></li>
                            <li class="breadcrumb-item active">Library Location</li>
                        </ol>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Daftar Library Location</h4>
                            </div>
                ...
                <form action="" id="form_data">
                    <div class="form-group">
                        ...
                        <input type="text" readonly name="id" id="id" class="form-control"
                            placeholder="ID Library Location">
                    </div>
                    <div class="form-group">
                        <label>Library :</label>
                        <select class="form-control" name="library_id" id="library_id">
                            @foreach ($library as $item)
                                <option {{ $item->id == $library_id ? 'selected' : '' }} value="{{ $item->id }}">
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                        {{-- <input type="text" name="library" id="library" class="form-control" placeholder="Library"> --}}
                    </div>
                    <div class="form-group">
                        <label>Location Name :</label>
                        <textarea name="name" id="name" class="form-control" placeholder="Location Name"></textarea>
                    </div>
                    <div class="form-group">
                        <label>Publish :</label>
...
<script>
    ...
    function loadDataTable() {
        $('#datatable_serverside').DataTable({
            ...
            pagingType: 'input',
            ajax: {
                url: '{{ url('admin/library_location/datatable') }}',
                method: 'POST',
                headers: {
    ...
    function create() {
        ...
        $.ajax({
            url: '{{ url('admin/library_location/create') }}',
            type: 'POST',
            dataType: 'JSON',
            data: data,
            headers: {
    ...
    function show(id) {
        toUpdate();
        $.ajax({
            url: '{{ url('admin/library_location/show') }}' + '/' + id,
            type: 'GET',
            dataType: 'JSON',
            beforeSend: function() {
                loadingOpen('.modal-content');
            ...
            success: function(response) {
                loadingClose('.modal-content');
                $('#id').val(response.id);
                $('#library_id').val(response.library_id);
    ...
    function update(id) {
        var data = $('#form_data').serializeArray();
        $.ajax({
            url: '{{ url('admin/library_location/update') }}' + '/' + id,
            type: 'POST',
            dataType: 'JSON',
            data: data,
            headers: {
    ...
    function destroy(id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: '{{ url('admin/library_location/destroy') }}' + '/' + id,
                    type: 'POST',
                    dataType: 'JSON',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
