  ...
  <div class="content-wrapper">
    <div class="content-header row">
      <div class="content-header-left col-md-6 col-12 mb-2">
        <h3 class="content-header-title mb-1 d-inline-block">Master Kelurahan</h3><br>
...
<script>
  ...
  function reset() {
    ...
    $('#district_idd').val('').trigger('change');
  ...
  function loadDataTable() {
    $('#datatable_serverside').DataTable({
      ...
      pagingType : 'input',
      ajax: {
        url: '{{ url("admin/village/datatable") }}'
      },
      columns: [
        {
          name: 'id',
  ...
  function show(id) {
    ...
    $.ajax({
      ...
      success: function(response) {
        loadingClose('.modal-content');
        $('#district_id').val(response.district_id).trigger('change');
  ...
  function update(id) {
    $.ajax({
      url: '{{ url("admin/village/update") }}' + '/' + id,
      type: 'POST',
      dataType: 'JSON',
      data: $('#form_data').serialize(),
      headers: {
  ...
  function destroy(id) {
    Swal.fire({
      ...
      if(result.value) {
        $.ajax({
          url: '{{ url("admin/village/destroy") }}' + '/' + id,
          type: 'GET',
          dataType: 'JSON',
          success: function(response) {
            if(response.status == 200) {
