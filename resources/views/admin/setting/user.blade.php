  ...
  <div class="content-wrapper">
    <div class="content-header row">
      <div class="content-header-left col-md-6 col-12 mb-2">
        <h3 class="content-header-title mb-1 d-inline-block">Pengaturan User</h3><br>
      ...
      <div class="modal-body">
        ...
        <form action="" id="form_data">
          <div class="form-group">
            <label>Nama :</label>
            <input type="text" name="fullname" id="fullname" class="form-control" placeholder="Masukan nama lengkap">
          </div>
          <div class="form-group">
            <label>Email :</label>
            <input type="text" name="email" id="email" class="form-control" placeholder="Masukan email valid">
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <label>Alamat :</label>
            <textarea name="address" id="address" style="resize:none;" class="form-control" placeholder="Masukan alamat lengkap"></textarea>
          </div>
          <div class="form-group">
            <label>Perpustakaan :</label>
            <select name="library_id" id="library_id" class="form-control select2" style="width:100%;">
...
<script>
  ...
  function show(id) {
    toUpdate();
    $.ajax({
      url: '{{ url("admin/setting/user/show") }}' + '/' + id,
      type: 'GET',
      dataType: 'JSON',
      beforeSend: function() {
        loadingOpen('.modal-content');
      ...
      success: function(response) {
        ...
        $('#library_id').val(response.library_id).trigger('change');
  ...
  function destroy(id) {
    Swal.fire({
      ...
      if(result.value) {
        $.ajax({
          url: '{{ url("admin/setting/user/destroy") }}' + '/' + id,
          type: 'GET',
          dataType: 'JSON',
          success: function(response) {
            if(response.status == 200) {
  ...
  function resetPassword(id) {
    Swal.fire({
      title: 'Password akan direset menjadi (eDepositV3)?',
      text: '',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      ...
      if(result.value) {
        $.ajax({
          url: '{{ url("admin/setting/user/reset_password") }}' + '/' + id,
          type: 'GET',
          dataType: 'JSON',
          success: function(response) {
            if(response.status == 200) {
