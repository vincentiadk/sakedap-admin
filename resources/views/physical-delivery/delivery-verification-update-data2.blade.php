                            ...
                            @if(count($collectionISBN) > 0)
                                @foreach($collectionISBN as $ci)
                                    ...
                                    <tr class="animate__animated animate__fadeIn">
                                        ...
                                        <td class="align-middle">
                                            <input type="text" class="form-control form-control-sm" name="ci_eks[]" value="{{ $ci->COPY ?? '' }}" placeholder="COPY" {{ $receivedBy ? 'readonly' : '' }}>
                                        </td>
                                        <td class="text-center align-middle">
                                            @if($receivedBy)
                                                <span class="badge bg-success-subtle text-success">Terverifikasi: {{ $receivedBy }}</span>
                                            @else
                                                <button type="button" class="btn btn-danger btn-sm" onclick="removeItem(this)" data-letter-detail="{{ $ci->LETTER_DETAIL_ID }}" data-bs-toggle="tooltip" title="Hapus item">
                                                    <i class="ph-trash"></i>
                                                </button>
                                            @endif
                                        </td>
    ...
    function removeItem(param) {
        swalInit.fire({
            ...
            if (result.isConfirmed) {
                ...
                var letter_detail_id = $(param).attr('data-letter-detail');
                ...
                $.ajax({
                        url: '{{ url("physical-delivery/delivery-verification2/destroy-data-ld") }}',
                        type: 'DELETE',
                        dataType: 'JSON',
                        data: {
                            id: letter_detail_id
